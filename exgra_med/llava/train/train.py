...
def preprocess(
    sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, is_val
) -> Dict:
    """
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        data_dict = preprocess(sources, self.tokenizer, self.is_val)
        if isinstance(i, int):
            data_dict = dict(
...
def make_supervised_data_module(
    ...
    if data_args.evaluate_on_test:
        ...
        folder, fname = os.path.split(data_args.data_path)
        test_path = os.path.join(folder, "test" + fname[5:])
        ...
        qa_type_list_test = []
        for qa in raw_test:
            qa_type_list_test.append(qa["answer_type"])
    ...
    if data_args.evaluate_on_val:
        ...
        folder, fname = os.path.split(data_args.data_path)
        val_path = os.path.join(folder, "val" + fname[5:])
        ...
        qa_type_list_val = []
        for qa in raw_val:
            qa_type_list_val.append(qa["answer_type"])
...
def prepare_compute_metrics(tokenizer, qa_type_dict, should_evaluate):
    ...
    def compute_metrics(eval_preds):
        ...
        for i, (prediction, label, qa_type) in enumerate(
            ...
            ans_end_idx = label.index("\n")
            gt_value = label[ans_start_idx + 10 : ans_end_idx].strip().lower()  # 'yes'
            ...
            try:
                ...
                prediction += "###"
                index = prediction.index("###")
            pred_value = prediction[:index].strip().lower()  # 'assistant: no'
            ...
            if qa_type == "OPEN":
                _, _, recall = calculate_f1score(pred_value, gt_value)
